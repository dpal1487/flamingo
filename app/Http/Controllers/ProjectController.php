...
class ProjectController extends Controller
{
    ...
    {
      $this->data['partners']=Partners::orderBy('id','desc')->get();
      if($request->ajax())
      {
      ...
      else
      {
          ...
          {
              $this->data['projects']=Projects::where('project_name', 'like', '%' . $request->keyword . '%')->orWhere('project_id', 'like', '%' . $request->keyword . '%')->orderBy('id','desc')->paginate(15);
          }
          else
          {
              $this->data['projects']=Projects::orderBy('id','desc')->paginate(15);
          }
      }
    ...
    {
        $this->data['partners']=Partners::orderBy('id','DESC')->get();
        $this->data['industries']=Industries::orderBy('name','asc')->get();
    ...
    public function addLinks(Request $request)
    {
      ...
      if($pro=Projects::where('id',$request->id)->first())
      {
          $data=array(
    ...
    {
      if(Projects::where('id',$request->id)->first())
      {
          $data=array(
          'client_live_url'=>$request->client_live_url,
          'client_test_url'=>$request->client_test_url,
          ...
          if(Projects::where(['id'=>$request->id])->update($data))
          {
    ...
    {
        $this->data['partners']=Partners::orderBy('id','DESC')->get();
        $this->data['project'] = Projects::where('id',$id)->first();
        $this->data['industries']=Industries::orderBy('name','asc')->get();
    ...
    public function projectId()
    {
        do {
           $id = str::random( 20 );
        } while ( Projects::where( 'id', $id )->exists() );
        return $id;
    }
    ...
    {
      if(Projects::where('id',$id)->update(['status' =>$request->status]))
      {
